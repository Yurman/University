...
public class StudentControllerTest {
	...
	mockMvc.perform(request).andExpect(status().isOk()).andExpect(view().name("student-templates/students"))
	...
	mockMvc.perform(request.param("id", "5")).andExpect(redirectedUrl("/students")).andExpect(status().isFound());
    }
	...
	mockMvc.perform(request.param("id", "2")).andExpect(view().name("student-templates/student-info"))
		.andExpect(status().isOk()).andExpect(model().attributeExists("student"));
    }
	...
	mockMvc.perform(request.param("id", "33")).andExpect(view().name("student-templates/student-info"))
	...
	mockMvc.perform(request.param("id", "2")).andExpect(view().name("student-templates/edit-student"))
		.andExpect(status().isOk()).andExpect(model().attributeExists("groups"))
	...
	mockMvc.perform(request.param("id", "3")).andExpect(view().name("student-templates/edit-student"))
		.andExpect(status().isOk()).andExpect(model().attributeExists("groups"));
    }
	...
	mockMvc.perform(request.requestAttr("studentDto", studentDto)).andExpect(redirectedUrl("/students"))
